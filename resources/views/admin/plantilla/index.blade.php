{{-- DESTINO: resources/views/admin/plantilla/index.blade.php --}}
...
.entrenador-content { font-family:'DM Sans',sans-serif; color:var(--text); }
...
.plantilla-nota { font-size:0.78rem; color:var(--muted); background:var(--accent-l); border:1px solid #bfdbfe; border-radius:8px; padding:8px 12px; margin-bottom:16px; }
...
<div class="plantilla-nota">
    Estos ejercicios se copian a cada entrenador nuevo cuando se registra.
    ...
    <div class="ejercicios-grid">
        ...
        <div class="ej-card" data-nombre="{{ strtolower($ej->nombre) }}">
            ...
            <div class="ej-card-actions">
                ...
                <form method="POST" action="{{ route('admin.plantilla.destroy',$ej->id) }}"
                      onsubmit="return confirm('¿Eliminar «{{ $ej->nombre }}» de la plantilla? Esta acción no se puede deshacer.')">
                    @csrf
                    @method('DELETE')
...
@empty
<div class="ej-empty">Aún no hay ejercicios en la plantilla. Agrega el primero con el botón de arriba ↑</div>
@endforelse
</div>
...
<div class="modal-overlay" id="modalEjercicio" onclick="if(event.target===this) cerrarModalEjercicio()">
    <div class="modal-box">
        ...
        <form id="formEjercicio" method="POST" enctype="multipart/form-data" action="{{ route('admin.plantilla.store') }}">
            @csrf
...
<script>
const EJ_UPDATE_URL = "{{ route('admin.plantilla.update', ['ejercicio' => 'EJID']) }}";
const EJ_STORE_URL  = "{{ route('admin.plantilla.store') }}";
