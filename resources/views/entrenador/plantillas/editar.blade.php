...
<div class="nombre-plantilla-wrap">
    ...
    <div>
        ...
        <textarea name="descripcion" class="desc-input" rows="2"
                  placeholder="Ej: Rutina de empuje para nivel intermedio">{{ $plantilla->descripcion }}</textarea>
    </div>
</div>
...
const bloquesExistentes  = @json($plantilla->bloques ?? []);
...
function buildEjercicioRow(grupo, i, ej) {
    ...
    const ejImagen       = ejActual?.imagen ? `/storage/${ejActual.imagen}` : '';
    const optsSegmento   = ej
        ? Object.keys(ejerciciosPorGrupo).map(s => `<option value="${s}" ${s === ej.segmento ? 'selected' : ''}>${s}</option>`).join('')
        : opts;
    const optsEjercicios = ej
        ? ejsSegmento.map(e => {
            const url = e.imagen ? `/storage/${e.imagen}` : '';
            const sel = String(e.id) === String(ej.ejercicio_id) ? 'selected' : '';
            return `<div class="ej-select-option ${sel}" data-value="${e.id}" data-nombre="${e.nombre}" data-imagen="${url}" onclick="seleccionarEjercicio(this)">
                ${url ? `<img src="${url}" alt="${e.nombre}">` : '<div class="ej-no-img">Sin img</div>'}
                <span>${e.nombre}</span>
            </div>`;
        }).join('')
        : '';
        ...
        <div class="ejercicio-row ${bgClass}">
            ...
            <div class="col-ejercicio">
                ...
                <div class="ej-select-wrapper" data-target="${ejId}">
                    ...
                    <div class="ej-select-dropdown">${optsEjercicios}</div>
                </div>
            </div>
...
function agregarBloque(tipo, cantidad, ejercsData = null) {
    const grupo = ejercsData ? Object.keys(ejercsData)[0]?.split('-')[0] ?? 'G' + contador++ : 'G' + contador++;
    ...
    for (let i = 0; i < cantidad; i++) {
        html += buildEjercicioRow(grupo, i, null);
    }
...
/* ══ PRECARGAR BLOQUES EXISTENTES ══ */
Object.entries(bloquesExistentes).forEach(([grupo, bloque]) => {
    const tipo     = bloque.tipo;
    const ejercs   = bloque.ejercicios ?? {};
    const cantidad = Object.keys(ejercs).length;
    ...
    Object.entries(ejercs).forEach(([i, ej]) => {
        html += buildEjercicioRow(grupo, parseInt(i), ej);
    });
    ...
    Object.entries(ejercs).forEach(([i, ej]) => {
        ...
        actualizarHeader(grupo, numSeries);
        const seriesInput = document.querySelector(`.bloque[data-grupo="${grupo}"] .bloque-series-count input`);
        if (seriesInput) seriesInput.value = numSeries;
