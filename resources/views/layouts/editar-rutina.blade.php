...
<div id="modalBorrarHistorial" onclick="if(event.target===this)cerrarModalBorrarHistorial()"
    ...
    <div style="background:white;border-radius:14px;width:100%;max-width:400px;box-shadow:0 20px 60px rgba(0,0,0,.2);">
        <div style="display:flex;align-items:center;justify-content:space-between;padding:16px 20px;border-bottom:1px solid #e2e5ea;border-radius:14px 14px 0 0;">
            ...
            <button onclick="cerrarModalBorrarHistorial()" style="width:28px;height:28px;border-radius:7px;background:#f3f4f6;border:none;cursor:pointer;font-size:1rem;color:#6b7280;">✕</button>
        </div>
...
<div class="page-header">
    <h2>{{ $cliente->name }}</h2>
    <span class="badge">Semana {{ $semanaVisual }} · Día {{ $dia }}</span>
...
<div class="flex items-center gap-1.5 mb-4 relative" id="wp-nav">
    ...
    <a href="{{ $nextSem<=$semanaFinPlan ? route('entrenador.rutina.editar',[$cliente->id,$nextSem,$nextDia]) : '#' }}"
       class="w-6 h-6 flex items-center justify-center rounded border border-gray-200 bg-gray-50 text-gray-400 hover:border-blue-400 hover:text-blue-500 transition-colors text-xs flex-shrink-0 {{ $nextSem>$semanaFinPlan ? 'opacity-30 pointer-events-none' : '' }}">›</a>
</div>
