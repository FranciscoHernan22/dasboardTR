  ...
  >
    ...
    <div class="px-3 py-3 border-t border-slate-700/50">
      <div class="flex items-center gap-3 px-3 py-2 mb-1">
        <div class="w-7 h-7 rounded-full bg-blue-600/30 flex items-center justify-center text-xs font-semibold text-blue-300 flex-shrink-0">
          {{ strtoupper(substr(auth()->user()->name ?? 'F', 0, 2)) }}
        </div>
        <div class="min-w-0">
          <p class="text-sm font-medium text-white whitespace-nowrap truncate">{{ auth()->user()->name ?? 'Francisco' }}</p>
          <p class="text-xs text-slate-500 whitespace-nowrap">Entrenador</p>
        </div>
      </div>
      <form method="POST" action="{{ route('entrenador.logout') }}">
        @csrf
        <button type="submit"
          class="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-slate-400 hover:bg-red-500/10 hover:text-red-400 transition-colors duration-100 whitespace-nowrap">
          <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="1.75" viewBox="0 0 24 24">
